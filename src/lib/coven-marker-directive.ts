...
export function buildCovenMarkersDirective(): string {
  return [
    ...
    'During an /auto mission, report your state with <coven:auto-status state="clarifying|working|blocked|done" note="short status" /> — clarifying while you still need answers from the human, working while proceeding silently, blocked the moment you need a human (permissions, a decision, credentials, anything you cannot resolve yourself) with the reason in note, done when the mission is finished with a one-line summary in note. Re-emit to update the card in place; only blocked and done should draw the human back into the chat.',
