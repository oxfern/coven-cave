...
assert.match(
  route,
  /const openCodeLaunchCommand = openCodeDirect \? openCodeLaunch\(spawnArgs\) : null;[\s\S]*?const spawnEnv = openCodeDirect\s*\? openCodeSpawnEnv\(body\.familiarId\)\s*: hermesDirect\s*\? readyHermesLaunch!\.env\s*: harnessSpawnEnv\(body\.familiarId\);[\s\S]*?child = spawn\(command\.command, command\.args, \{[\s\S]*?env: spawnEnv,[\s\S]*?writeOpenCodeLaunchInput\(child, openCodeLaunchCommand\)/,
