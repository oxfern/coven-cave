...
assert.match(
  chatRoute,
  /const harnessSessionId = grokDirect\s*\?\s*grokSessionId\s*:\s*binding\.harness === "codex"\s*\?\s*codexHarnessSessionId \?\? existingConversation\?\.harnessSessionId \?\? sessionId\s*:\s*sessionId;/,
