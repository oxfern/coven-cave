...
assert.match(model, /caveCrafts\(\)[\s\S]*\? \[\{ id: "crafts", label: "Crafts"/, "the Crafts tab is flag-gated");
...
assert.match(view, /visibleMarketplacePlugins\(plugins, craftsEnabled\)/, "Craft cards stay out of Yours while the flag is off");
assert.match(view, /const visiblePlugins = useMemo\(\s*\(\) => visibleMarketplacePlugins\(plugins, craftsEnabled\)/, "Yours derives one craft-filtered inventory");
assert.match(view, /categoriesFrom\(visiblePlugins\)/, "Yours categories use the craft-filtered inventory");
assert.match(view, /for \(const p of visiblePlugins\)/, "Yours category counts use the craft-filtered inventory");
assert.match(view, /cat === "All" \? visiblePlugins\.length/, "Yours All count excludes hidden Crafts");
assert.match(view, /initialSection === "crafts"[\s\S]*\? craftsEnabled \? "crafts" : "browse"/, "Craft deep links fall back to Yours while the flag is off");
